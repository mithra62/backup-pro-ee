...
<?php 
	if(count($storage_details) > 0):
    	...
    	foreach($storage_details AS $key => $storage): 
    	   $row = array(
    	       '<a href="'.$url_base.'edit_storage&id='.$key.'">'.$view_helper->m62Escape($storage['storage_location_name']).'</a>',
    	       '<img src="'.$theme_folder_url.'backup_pro/images/storage/'.$storage['storage_location_driver'].'.png" class="" title="'.$view_helper->m62Escape($storage['storage_location_name']).'">',
