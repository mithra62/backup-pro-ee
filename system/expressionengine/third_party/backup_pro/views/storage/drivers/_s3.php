...
$this->table->add_row(
    ...
    form_input('s3_access_key', $form_data['s3_access_key'], 'id="s3_access_key"').$view_helper->m62FormErrors($form_errors['s3_access_key'])
);
...
$this->table->add_row(
    ...
    form_password('s3_secret_key', $form_data['s3_secret_key'], 'id="s3_secret_key"').$view_helper->m62FormErrors($form_errors['s3_secret_key'])
);

$this->table->add_row(
    '<label for="s3_bucket">'.$view_helper->m62Lang('s3_bucket').'</label><div class="subtext">'.$view_helper->m62Lang('s3_bucket_instructions').'</div>', 
    form_input('s3_bucket', $form_data['s3_bucket'], 'id="s3_bucket"').$view_helper->m62FormErrors($form_errors['s3_bucket'])
);
...
$this->table->add_row(
    ...
    form_checkbox('s3_reduced_redundancy', '1', $form_data['s3_reduced_redundancy'], 'id="s3_reduced_redundancy"').$view_helper->m62FormErrors($form_errors['s3_reduced_redundancy'])
);
